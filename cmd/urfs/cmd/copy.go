...
var copyDescription = "copies a urchinfs object to another location."
...
var copyCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if isUrfsURL(source) {
			endpoint, bucketName, objectKey, err := parseUrfsURL(source)
			if err != nil {
				return err
			}

			if err := copyObjectStorageToLocalFile(ctx, cfg, endpoint, bucketName, objectKey, target); err != nil {
				return err
			}
		...
		_, bucketName, objectKey, err := parseUrfsURL(target)
		if err != nil {
			return err
		}
...
func validateCopyArgs(args []string) error {
	if isUrfsURL(args[0]) && isUrfsURL(args[1]) {
		return errors.New("source and target url cannot both be urfs:// protocol")
	}
...
func copyObjectStorageToLocalFile(ctx context.Context, cfg *config.DfstoreConfig, endpoint, bucketName, objectKey, filePath string) error {
	...
	meta, err := dfs.GetObjectMetadataWithContext(ctx, &urfs.GetUrfsMetadataInput{
		Endpoint:   endpoint,
	...
	reader, err := dfs.GetObjectWithContext(ctx, &urfs.GetUrfsInput{
		Endpoint:   endpoint,
		BucketName: bucketName,
		ObjectKey:  objectKey,
	})
	...
	fmt.Printf("download object storage success, length: %d bytes cost: %d ms\n", meta.ContentLength, time.Since(start).Milliseconds())
	return nil
}
...
func copyLocalFileToObjectStorage(ctx context.Context, cfg *config.DfstoreConfig, bucketName, objectKey, filepath string) error {
	...
	if err := urfs.New(cfg.Endpoint).PutObjectWithContext(ctx, &urfs.PutObjectInput{
		BucketName:  bucketName,
		ObjectKey:   objectKey,
		Filter:      cfg.Filter,
		Mode:        cfg.Mode,
	...
	fmt.Printf("upload object storage success, length: %d bytes cost: %d ms\n", size, time.Since(start).Milliseconds())
	return nil
}
